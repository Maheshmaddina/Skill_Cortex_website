...
- With `JOBS_TICK_ENABLED=true`, open browsers ping `POST /api/internal/tick` every 2 minutes and the server runs the jobs at most once a minute (claimed in Postgres), so jobs keep running while the site has visitors even though GitHub's schedule is often hours late. For quiet periods, an external cron (e.g. cron-job.org) calling `run-jobs` every 5 minutes is the reliable backstop.
